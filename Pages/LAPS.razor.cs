        ...
        private async Task ClearLapsPassword(AdComputer computer)
        {
            try
            {
                ...
                if (tab != null && computer.LapsInformations != null)
                {
                    ...
                    {
                        ["ContentText"] = $"Clear LAPS {version} Password on Computer '{computer.Name}' ?{Environment.NewLine}You have to invoke gpupdate /force on computer '{computer.Name}' in order so set a new LAPS password",
        ...
        private async Task FetchComputerDetailsAsync(string distinguishedName, string computerName)
        {
            try
            {
                ...
                if (adComputerObject != null && selectedComputer != null)
                {
                    ...
                    // АУДИТ ПЕРЕГЛЯДУ ПАРОЛЯ
                    try
                    {
                        var username = (await SessionManager.GetLdapCredentialsAsync())?.UserName ?? "Невідомо";
                        await AuditService.LogAsync(username, "Перегляд пароля", $"Комп'ютер: {computerName}");
                    }
                    catch
                    {
                        // Не зупиняємо, якщо аудит не вдався
                    }
